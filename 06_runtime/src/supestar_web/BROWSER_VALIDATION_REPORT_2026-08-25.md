# 수페스타 Web Runtime 외부 브라우저 검증 보고서
...
4174 서버를 최신 코드로 재시작한 뒤 같은 브라우저 대화창에서 순서대로 직접 입력했다.

| 순서 | 입력 | 확인 결과 |
| --- | --- | --- |
| 1 | `ESG가 무엇인가요?` | ESG의 환경·사회·지배구조만 설명 |
| 2 | `SDGs는요?` | 직전 ESG를 합치지 않고 `지속가능발전목표(SDGs)`만 답변 |
| 3 | `그건 왜 중요한가요?` | 명시적 지시어를 인식해 가장 최근 사용자 주제인 SDGs를 이어서 답변 |

라우터·KAC·Context·AI는 공통 `EXPLICIT_FOLLOW_UP_ONLY` 정책을 사용한다. 새 주제는 이전 발화 사용량 0, 명시적 후속 질문은 직전 사용자 발화 사용량 1로 기록되며 assistant 답변은 재입력하지 않는다. 결정론적 63건과 실제 로컬 AI 10건의 manifest에서도 동일 정책을 검증했다.
...
기존 결함은 재현 기록과 수정 후 기록으로 구분된다. 현재 4174 서버는 대화 이력 격리 수정까지 반영한 코드이며, 새 주제는 이전 질문과 분리하고 명시적 후속 질문만 직전 사용자 발화를 연결한다. 구체 질문은 구조화된 원자 Skill 입력으로 실제 후보를 답하고, 불충분·상충·외부 주장·우회 질문은 모델 문장을 사용하지 않는다.
